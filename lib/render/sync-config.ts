...
/**
 * Rebuild OpenClaw config from DB and push it to the running Render container.
 * This updates environment variables and redeploys the service.
 * Call this after any config/channel change from the dashboard.
 */
export async function syncConfigToRender(instanceId: string): Promise<void> {
  ...
  await render.updateEnvVars(instance.containerId, envVars)
  await render.deploy(instance.containerId)
